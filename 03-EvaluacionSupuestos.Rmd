...
```{r bass, echo=T, fig.height = 4, fig.width = 5,fig.align = "center",fig.cap = "\\label{fig:bass} Relación entre la edad y la longitud de los peces lobina boca chica.",warning=FALSE,message = FALSE}
library(alr4)
data("wblake")
plot(Length~Age,data=wblake,xlab='edad (años)',ylab='longitud (mm)')
```

La Figura \ref{fig:bass} muestra que la relación entre estas dos variables se puede aproximar a una recta, por lo cuál se propone el siguiente:
\[
...
b. $Var(\varepsilon_{j})=\sigma^{2}$. Homogeneidad de varianza en los errores,
c. $Cov(\varepsilon_{i},\varepsilon_{j})=0$ para todo $i\neq j$. Los errores están incorrelacionados,
d. $\varepsilon_{j} \sim Normal(0, \sigma^{2})$. Los errores se distribuyen de forma normal.
...
```{r PatronesResiduos, echo=F, fig.height = 5, fig.width = 7,fig.align = "center",fig.cap = "Ejemplos de posibles patrones de residuos",warning=FALSE,message = FALSE}
set.seed(1)
x1= runif(100,0,1)
e1 = rnorm(100)
set.seed(4)
...
```{r pecesResiduos1, message=FALSE, warning=FALSE, fig.height = 3.5, fig.width = 4,fig.align = "center",fig.cap = "Datos de la longitud de los peces. Gráfico de los residuos estudentizados contra los valores ajustados."}
