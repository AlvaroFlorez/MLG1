...
\chapter{Modelo lineal simple}\label{modelo-lineal-simple}}

\hypertarget{datos-de-peso-al-nacer}{%
\section*{Datos de peso al nacer}\label{datos-de-peso-al-nacer}}
\addcontentsline{toc}{section}{Datos de peso al nacer}
...
\section{Regresion lineal simple}\label{regresion-lineal-simple}}
...
\section{Modelo lineal simple}\label{modelo-lineal-simple-1}}
...
\subsection*{Datos de peso al nacer. Modelo y estimación de parametros}\label{datos-de-peso-al-nacer.-modelo-y-estimaciuxf3n-de-parametros}}
\addcontentsline{toc}{subsection}{Datos de peso al nacer. Modelo y estimación de parametros}
...
A partir de \(\hatbeta_{1}\), podemos concluir que la edad gestacional tiene un efecto positivo sobre el peso del recién nacido (el coeficiente estimado es positivo). Por cada incremento de una semana en la edad gestacional, el valor esperado del peso del recién nacido aumenta 0.18 kilogramos.
...
\section{Propiedades de los estimadores por MCO}\label{propiedades-de-los-estimadores-por-mco}}
...
\label{eq:H0beta1}
\end{equation}
Dado que \(\hatbeta_{1}\) es una combinación lineal de \(y_{i}\) \eqref{eq:beta1cl}, podemos concluir que:
\[
...
\subsection*{Datos de peso al nacer. Pruebas de hipótesis y ANOVA}\label{datos-de-peso-al-nacer.-pruebas-de-hipuxf3tesis-y-anova}}
\addcontentsline{toc}{subsection}{Datos de peso al nacer. Pruebas de hipótesis y ANOVA}
...
\subsubsection*{\texorpdfstring{Intervalos de confianza para \(\beta_{0}\), \(\beta_{1}\) y \(\sigma^{2}\)}{Intervalos de confianza para \textbackslash beta\_\{0\}, \textbackslash beta\_\{1\} y \textbackslash sigma\^{}\{2\}}}\label{intervalos-de-confianza-para-beta_0-beta_1-y-sigma2}}
\addcontentsline{toc}{subsubsection}{Intervalos de confianza para \(\beta_{0}\), \(\beta_{1}\) y \(\sigma^{2}\)}

Los intervalos de confianza para \(\beta_{0}\) y \(\beta_{1}\) se construyen a partir de las distribuciones de probabilidad de \(\hatbeta_{0}\) y \(\hatbeta_{1}\). Esto es, \eqref{eq:distB0} y \eqref{eq:distB1}, respectivamente.
...
\subsubsection*{\texorpdfstring{Intervalos de confianza para \(E(y_{i})\) y una predicción futura}{Intervalos de confianza para E(y\_\{i\}) y una predicción futura}}\label{intervalos-de-confianza-para-ey_i-y-una-predicciuxf3n-futura}}
\addcontentsline{toc}{subsubsection}{Intervalos de confianza para \(E(y_{i})\) y una predicción futura}
...
\subsection*{Datos de peso al nacer. Intervalos de confianza}\label{datos-de-peso-al-nacer.-intervalos-de-confianza}}
\addcontentsline{toc}{subsection}{Datos de peso al nacer. Intervalos de confianza}
...
\section{Estimador por máxima verosimilitud}\label{estimador-por-muxe1xima-verosimilitud}}
...
\chapter{Modelo lineal múltiple}\label{modelo-lineal-muxfaltiple}}

\rule{\textwidth}{0.4pt}

\hypertarget{bajo-peso-al-nacer}{%
\section*{Bajo peso al nacer}\label{bajo-peso-al-nacer}}
\addcontentsline{toc}{section}{Bajo peso al nacer}
...
\section{Modelo lineal múltiple}\label{modelo-lineal-muxfaltiple-1}}
...
\section{Estimación de los parámetros de regresión}\label{estimaciuxf3n-de-los-paruxe1metros-de-regresiuxf3n}}
...
\[
S(\bbeta) =  \sum_{i=1}^{n}\left(y_{i} - \bx_{i}'\bbeta \right)^2 = \sum_{i=1}^{n}\epsilon_{i}^2.
\]
En forma matricial, tenemos:
\begin{equation}
\begin{split}
S(\bbeta) &= \bvarepsi'\bvarepsi = (\by - \bX\bbeta)'(\by - \bX\bbeta) \\
...
\]
La matriz \((n\times n)\) \(\bH = \bX(\bX'\bX)^{-1}\bX'\) es llamada \textbf{matriz hat} (sombrero) y desempeña un papel importante en el análisis de regresión.
...
\subsection{\texorpdfstring{Estimación de \(\sigma^{2}\)}{Estimación de \textbackslash sigma\^{}\{2\}}}\label{estimaciuxf3n-de-sigma2-1}}
...
\[
E(SS_{res}) = E\left[\by'(\bI - \bH)\by\right] = \sigma^{2}\tr\left( \bI - \bH \right) + \bbeta'\bX'(\bI - \bH)\bX\bbeta = (n-p)\sigma^{2}.
\]
...
\subsection{Bajo peso al nacer - estimación de parámetros}\label{bajo-peso-al-nacer---estimaciuxf3n-de-paruxe1metros}}
...
\subsection{Propiedades de los estimadores por MCO}\label{propiedades-de-los-estimadores-por-mco-1}}
...
\subsubsection*{Teorema de Gauss-Markov}\label{teorema-de-gauss-markov}}
\addcontentsline{toc}{subsubsection}{Teorema de Gauss-Markov}
...
\subsection{Análisis de varianza}\label{anuxe1lisis-de-varianza-1}}
...
\subsection{Pruebas individuales sobre los coeficientes}\label{pruebas-individuales-sobre-los-coeficientes}}
...
\subsection{Pruebas sobre subconjuntos de coeficientes}\label{pruebas-sobre-subconjuntos-de-coeficientes}}
...
\]
donde \(\bX_{1}\) es la matriz \(n\times (p-r)\) que contiene las columnas de \(\bX\) asociadas con \(\bbeta_{1}\), y \(\bX_{2}\) es la matriz \(n\times r\) que contiene las columnas de \(\bX\) asociadas con \(\bbeta_{2}\). Este es llamado el \textbf{modelo completo}.
...
Para evaluar la contribución de los regresores asociados a \(\bbeta_{2}\), ajustamos el modelo asumiendo que \(H_{0}\) es cierta. De esta forma tenemos el \textbf{modelo reducido}:
\[
...
\section{Prueba de hipótesis lineal general}\label{prueba-de-hipuxf3tesis-lineal-general}}
...
El \textbf{modelo reducido (RM)} se obtiene al resolver las \(r\) ecuaciones independientes de \(\bT\bbeta = \bZERO\) para los \(r\) coeficientes en el modelo completo en términos de los \(p-r\) coeficientes restantes. Esto lleva al siguiente RM:
\[
...
\subsection*{Bajo peso al nacer - pruebas de hipótesis}\label{bajo-peso-al-nacer---pruebas-de-hipuxf3tesis}}
\addcontentsline{toc}{subsection}{Bajo peso al nacer - pruebas de hipótesis}
...
\subsection{\texorpdfstring{Intervalos de confianza para \(\beta_{j}\)}{Intervalos de confianza para \textbackslash beta\_\{j\}}}\label{intervalos-de-confianza-para-beta_j}}
...
\subsection{\texorpdfstring{Intervalos de confianza para el valor esperado de \(Y\) y una observación futura}{Intervalos de confianza para el valor esperado de Y y una observación futura}}\label{intervalos-de-confianza-para-el-valor-esperado-de-y-y-una-observaciuxf3n-futura}}
...
\subsection*{Bajo peso al nacer - intervalos de confianza}\label{bajo-peso-al-nacer---intervalos-de-confianza}}
\addcontentsline{toc}{subsection}{Bajo peso al nacer - intervalos de confianza}
...
\section{Extrapolación oculta en regresión múltiple}\label{extrapolaciuxf3n-oculta-en-regresiuxf3n-muxfaltiple}}
...
\subsection*{Bajo peso al nacer - interpolación}\label{bajo-peso-al-nacer---interpolaciuxf3n}}
\addcontentsline{toc}{subsection}{Bajo peso al nacer - interpolación}
...
Para la predicción en el punto \(\bx_{01}\), tenemos que \(h_{0} = (1, 34, 77)(\bX'\bX)^{-1}(1, 34, 77)' = 0.3487\) \(>\) \(h_{max} = 0.2276\). Por lo tanto, aquí se estaría extrapolando. Para el resto de punto no hay problemas de extrapolación.

\rule{\textwidth}{0.4pt}

\hypertarget{coeficientes-normalizados-de-regresiuxf3n}{%
\subsection{Coeficientes normalizados de regresión}\label{coeficientes-normalizados-de-regresiuxf3n}}
...
\subsubsection{Escalamiento de longitud unitaria}\label{escalamiento-de-longitud-unitaria}}
...
\subsection*{Bajo peso al nacer - coeficientes de regresión con variables escaladas}\label{bajo-peso-al-nacer---coeficientes-de-regresiuxf3n-con-variables-escaladas}}
\addcontentsline{toc}{subsection}{Bajo peso al nacer - coeficientes de regresión con variables escaladas}
...
\subsection{Bajo peso al nacer - factores de inflación de varianza}\label{bajo-peso-al-nacer---factores-de-inflaciuxf3n-de-varianza}}
...
\chapter{Evaluación de los supuestos del modelo}\label{evaluaciuxf3n-de-los-supuestos-del-modelo}}

\rule{\textwidth}{0.4pt}

\hypertarget{ejemplo-1.-datos-de-peso-al-nacer}{%
\section*{Ejemplo 1. Datos de peso al nacer}\label{ejemplo-1.-datos-de-peso-al-nacer}}
\addcontentsline{toc}{section}{Ejemplo 1. Datos de peso al nacer}
...
\section*{Ejemplo 2. Ventas de helados}\label{ejemplo-2.-ventas-de-helados}}
\addcontentsline{toc}{section}{Ejemplo 2. Ventas de helados}
...
\section*{Ejemplo 3. Longitud del pez lobina boca chica}\label{ejemplo-3.-longitud-del-pez-lobina-boca-chica}}
\addcontentsline{toc}{section}{Ejemplo 3. Longitud del pez lobina boca chica}
...
\section{Supuestos del modelo linea múltiple}\label{supuestos-del-modelo-linea-muxfaltiple}}
...
\section{Efectos del incumplimiento de los supuestos}\label{efectos-del-incumplimiento-de-los-supuestos}}

\hypertarget{sesgo-por-omisiuxf3n-de-variables-relevantes}{%
\subsection{Sesgo por omisión de variables relevantes}\label{sesgo-por-omisiuxf3n-de-variables-relevantes}}
...
\[
E(\hatsigma^{2}_{1}) = \sigma^{2} + \frac{\bbeta_{2}'\bX'_{2}(\bI_{n} - \bH_{1})\bX_{2}\bbeta_{2}}{n-p_{1}}.
\]
Dado que \((\bI - \bH_{1})\) es idempotente y, por lo tanto, positiva semi-definida, entonces \(E(\hatsigma_{1}^{2}) > \sigma^{2}\). Esto quiere decir que \(\hatsigma^{2}_{1}\) es un estimador sesgado de \(\sigma^{2}\)
...
\subsection{Incorrecta matriz de varianzas de los errores}\label{incorrecta-matriz-de-varianzas-de-los-errores}}
...
\end{pmatrix},
\]
tenemos que hay \textbf{heterocedasticidad}. Cada error tiene una varianza diferente \(V(\varepsilon_{j})=\sigma^{2}v_{jj}\), pero están incorrelacionados. Si los valores fuera de la diagonal de \(\bV\) son diferentes de cero, entonces los errores están correlacionados.
...
\subsection{Distribución no normal de los errores}\label{distribuciuxf3n-no-normal-de-los-errores}}
...
\subsection{Residuos estudentizados}\label{residuos-estudentizados}}
...
\subsection{Residuos PRESS y R-student}\label{residuos-press-y-r-student}}
...
\[
\hatsigma^{2}_{(i)} = \frac{(n-p)\hatsigma^{2} - \epsilon^{2}_{i}/(1-h_{ii})}{n-p-1}.
\]

\hypertarget{evaluaciuxf3n-del-cumplimiento-de-los-supuestos}{%
\section{Evaluación del cumplimiento de los supuestos}\label{evaluaciuxf3n-del-cumplimiento-de-los-supuestos}}
...
\subsubsection*{Bajo peso al nacer - gráfico de residuos}\label{bajo-peso-al-nacer---gruxe1fico-de-residuos}}
\addcontentsline{toc}{subsubsection}{Bajo peso al nacer - gráfico de residuos}
...
\subsection{Gráficos de residuos parciales}\label{gruxe1ficos-de-residuos-parciales}}
...
\[
e^{*}_{i}(y|x_{j}) = e_{i} - \hatbeta_{j}x_{ij}, \mbox{ para } i=1,\ldots,n.
\]
...
\subsubsection*{Bajo peso al nacer - gráfico de residuos parciales}\label{bajo-peso-al-nacer---gruxe1fico-de-residuos-parciales}}
\addcontentsline{toc}{subsubsection}{Bajo peso al nacer - gráfico de residuos parciales}
...
\subsection{Gráficos de normalidad}\label{gruxe1ficos-de-normalidad}}
...
\subsubsection*{Bajo peso al nacer - gráfico de normalidad}\label{bajo-peso-al-nacer---gruxe1fico-de-normalidad}}
\addcontentsline{toc}{subsubsection}{Bajo peso al nacer - gráfico de normalidad}
...
\subsection{Gráfico de residuos frente a el tiempo}\label{gruxe1fico-de-residuos-frente-a-el-tiempo}}
...
\subsubsection*{Consumo de helado - gráfico de los residuos contra el tiempo}\label{consumo-de-helado---gruxe1fico-de-los-residuos-contra-el-tiempo}}
\addcontentsline{toc}{subsubsection}{Consumo de helado - gráfico de los residuos contra el tiempo}
...
\section{Pruebas de hipótesis para evaluar los supuestos}\label{pruebas-de-hipuxf3tesis-para-evaluar-los-supuestos}}

\hypertarget{prueba-de-falta-de-ajuste}{%
\subsection{Prueba de falta de ajuste}\label{prueba-de-falta-de-ajuste}}
...
\subsubsection{Base de datos de la longitud de los peces - prueba de falta de ajuste}\label{base-de-datos-de-la-longitud-de-los-peces---prueba-de-falta-de-ajuste}}
...
\subsection{Prueba de heterocedasticidad}\label{prueba-de-heterocedasticidad}}
...
\subsubsection*{Bajo peso al nacer - prueba de heterocedasticidad}\label{bajo-peso-al-nacer---prueba-de-heterocedasticidad}}
\addcontentsline{toc}{subsubsection}{Bajo peso al nacer - prueba de heterocedasticidad}
...
\subsection{Prueba de normalidad}\label{prueba-de-normalidad}}
...
\subsubsection*{Bajo peso al nacer - prueba de normalidad}\label{bajo-peso-al-nacer---prueba-de-normalidad}}
\addcontentsline{toc}{subsubsection}{Bajo peso al nacer - prueba de normalidad}
...
\subsection{Prueba de correlación temporal de los errores}\label{prueba-de-correlaciuxf3n-temporal-de-los-errores}}
...
\subsubsection*{Ventas de helado - prueba de correlación temporal}\label{ventas-de-helado---prueba-de-correlaciuxf3n-temporal}}
\addcontentsline{toc}{subsubsection}{Ventas de helado - prueba de correlación temporal}
...
\chapter{Transformaciones y mínimos cuadrados ponderados}\label{transformaciones-y-muxednimos-cuadrados-ponderados}}

\rule{\textwidth}{0.4pt}

\hypertarget{ejemplo-1.-datos-de-la-onu}{%
\section*{Ejemplo 1. Datos de la ONU}\label{ejemplo-1.-datos-de-la-onu}}
\addcontentsline{toc}{section}{Ejemplo 1. Datos de la ONU}
...
\section*{Ejemplo 2. Datos de educación}\label{ejemplo-2.-datos-de-educaciuxf3n}}
\addcontentsline{toc}{section}{Ejemplo 2. Datos de educación}
...
\section{Transformación de los datos}\label{transformaciuxf3n-de-los-datos}}
...
\subsection{Transformaciones para linealizar el modelo}\label{transformaciones-para-linealizar-el-modelo}}
...
\subsection{Transformaciones para estabilizar la varianza}\label{transformaciones-para-estabilizar-la-varianza}}
...
\subsection*{Datos de la ONU. Transformación para linealizar los datos}\label{datos-de-la-onu.-transformaciuxf3n-para-linealizar-los-datos}}
\addcontentsline{toc}{subsection}{Datos de la ONU. Transformación para linealizar los datos}
...
\section{Método de Box-Cox}\label{muxe9todo-de-box-cox}}
...
\subsection{Datos de educación. Transformación de Box-Cox}\label{datos-de-educaciuxf3n.-transformaciuxf3n-de-box-cox}}
...
Estos resultados nos indican que \(\hatlambda = -1.09\). Por lo cuál, podemos utilizar una transformación inversa \((\lambda=-1)\). Entonces, el modelo propuesto es:
\[
...
\section{Mínimos cuadrados ponderados}\label{muxednimos-cuadrados-ponderados}}
...
La función de verosimilitud del modelo \eqref{eq:modV}, asumiendo que \(\bV\) es conocida, es:
\[
...
\subsection*{Datos de educación. Minímos cuadrados ponderados}\label{datos-de-educaciuxf3n.-minuxedmos-cuadrados-ponderados}}
\addcontentsline{toc}{subsection}{Datos de educación. Minímos cuadrados ponderados}
...
\chapter{Evaluación de puntos influyentes y atípicos}\label{evaluaciuxf3n-de-puntos-influyentes-y-atuxedpicos}}

\rule{\textwidth}{0.4pt}

\hypertarget{datos-de-la-onu}{%
\section{Datos de la ONU}\label{datos-de-la-onu}}
...
\section{Importancia de detectar valores influyentes y atípicos}\label{importancia-de-detectar-valores-influyentes-y-atuxedpicos}}
...
En la Figura \ref{fig:puntosAIw}(a) vemos que un punto atípico no tiene un efecto grande en la estimación de la recta de regresión. Sin embargo, dado que los puntos atípicos generan valores altos (en valor absoluto) para los residuos, estas observaciones inflan la varianza de las estimaciones y afectan las inferencias. En este caso, la estimación de la varianza con todos los datos es de \(\hatsigma^{2}=6.37\), y si se omite el dato \(A\), tenemos que \(\hatsigma^{2}=1.55\). Como vemos en la Figura \ref{fig:puntosAIw}(b) los puntos de balanceo tampoco tienen mucha influencia sobre las estimaciones por MCO ya que estos están en línea con el resto de los datos.

Por el otro lado, en la Figura \ref{fig:puntosAIw}(c) vemos que los puntos influyentes afectan notablemente las estimaciones por MCO. Además, también inflan considerablemente la variabilidad. Con todos los datos tenemos que \(\hatsigma^{2}=3.09\). Mientras que \(\hatsigma^{2}=1.55\) al eliminar la observación C.
...
\subsection*{Datos de la ONU - valores atípicos}\label{datos-de-la-onu---valores-atuxedpicos}}
\addcontentsline{toc}{subsection}{Datos de la ONU - valores atípicos}
...
\section{Puntos de balanceo}\label{puntos-de-balanceo}}
...
Además, la diagonal de la matriz \(\bH\) es una medida estandarizada de la distancia de las observaciones al centro (centroide) del espacio de \(\bx\). Por lo tanto, valores altos en la diagonal de \(\bH\) pueden indicarnos observaciones que son potencialmente influyentes porque están alejadas en el espacio de las covariables. Viendo \eqref{eq:hij}, si \(h_{ii}\) es muy cercano a uno, \(\haty_{i}\) estará muy cerca de \(y_{i}\) (dado que el peso de las demás observaciones será casi cero).
...
\subsection*{Datos de la ONU - diagonal de la matrix hat}\label{datos-de-la-onu---diagonal-de-la-matrix-hat}}
\addcontentsline{toc}{subsection}{Datos de la ONU - diagonal de la matrix hat}

La Figura \ref{fig:Un11Hdiag}(a) muestra los valores de la diagonal de la matrix \(\bH\). Aquí podemos observar que algunos países presentan valores más altos del punto de corte (\(2\frac{p}{n} = 0.0302\)). Comparado con los demás valores, el valor asociado a Trinidad y Tobago es muy alto. Por lo cuál este país lo podemos considerar como un punto de balanceo.
...
\subsection{Medidas de influencia}\label{medidas-de-influencia}}
...
\subsubsection{COVRATIO}\label{covratio}}
...
\subsubsection*{Datos de la ONU - medidas de influencia}\label{datos-de-la-onu---medidas-de-influencia}}
\addcontentsline{toc}{subsubsection}{Datos de la ONU - medidas de influencia}
