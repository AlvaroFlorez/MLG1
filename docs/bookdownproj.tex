...
\newcommand{\bvarepsi}{\bm{\varepsilon}}
...
{\centering \includegraphics{bookdownproj_files/figure-latex/birthweightFigure-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/gendataFigure-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Figresiduos-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/FigMCO2-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
 \FunctionTok{sqrt}\NormalTok{(}\FunctionTok{sum}\NormalTok{(mod}\SpecialCharTok{$}\NormalTok{residuals}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}\SpecialCharTok{/}\DecValTok{22}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics{bookdownproj_files/figure-latex/anovafig-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/birthweightIC-1} 
...
\begin{equation}
\begin{split}
L(\bm{\theta}) =& \prod_{i=1}^{n} \left(\sqrt{2\pi\sigma^{2}} \right)^{-1/2}\exp\left[ - \frac{1}{2\sigma^{2}}(y_{i}-\beta_{0}-\beta_{1}x_{i})^{2} \right] \\
=& \left( \sqrt{2\pi\sigma^{2}} \right)^{-n/2}\exp\left[ - \frac{1}{2\sigma^{2}} \sum_{i=1}^{n}(y_{i}-\beta_{0}-\beta_{1}x_{i})^{2}\right],
\end{split}
\nonumber
\end{equation}
para \(\bm{\theta}=(\beta_{0},\beta_{1},\sigma^{2})'\). La log-verosimilitud es:
\[
\ell (\bm{\theta}) = - \left(\frac{n}{2}\right)\left[ \log (2 \pi) - \log \sigma^{2}\right] - \frac{1}{2\sigma^{2}}\sum_{i=1}^{n}(y_{i} - \beta_{0}-\beta_{1}x_{i})^{2}. 
\]
El estimador de máxima verosimilitud de \(\bm{\theta}\) debe satisfacer:
\[
\left. \frac{\partial \ell(\bm{\theta})}{\partial \beta_{0}} \right|_{\widetilde{\bm{\theta}}} = \frac{1}{\widetilde{\sigma}^{2}}\sum_{i=1}^{n}(y_{i} - \beta_{0}-\beta_{1}x_{i}) = 0,
\]
\[
\left. \frac{\partial \ell(\bm{\theta})}{\partial \beta_{1}} \right|_{\widetilde{\bm{\theta}}} = \frac{1}{\widetilde{\sigma}^{2}}\sum_{i=1}^{n}(y_{i} - \beta_{0}-\beta_{1}x_{i})x_{i} = 0,
\]
y
\[
\left. \frac{\partial \ell(\bm{\theta})}{\partial \sigma^{2}} \right|_{\widetilde{\bm{\theta}}} = -\frac{n}{2\widetilde{\sigma}^{2}} + \frac{n}{2\widetilde{\sigma}^{4}}
...
{\centering \includegraphics{bookdownproj_files/figure-latex/extrapolacion-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/influyentes-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/BWdataFig-1} 
...
=& \bm{x}_{i}'\bm{\beta}+ \varepsilon_{i}, \qquad i=1,\ldots,n, \\
\end{split}
\label{eq:modMultiple}
\end{equation}

donde \(\bm{x}_{i} = (1,x_{i1},x_{i2},\ldots,x_{i,p-1})'\) es el vector de dimensión \(p\) de covariables del individuo \(i\) y \(\bm{\beta}= (\beta_{0},\beta_{1},\ldots,\beta_{p-1})'\) es el vector de dimensión \(p\) de coeficientes de regresión.
...
\begin{equation}
E(Y| x_{i1},x_{i2},\ldots,x_{i,p-1}) = E(Y| \bm{x}_{i}) = \beta_{0} + \beta_{1}x_{i1} + \beta_{2}x_{i2} + \ldots + \beta_{p-1} x_{i,p-1} = \bm{x}_{i}'\bm{\beta}.
\label{eq:expValue}
\end{equation}
...
\end{pmatrix}, & \bm{\varepsilon}= \begin{pmatrix}
\varepsilon_{1} \\ \varepsilon_{2} \\ \vdots \\ \varepsilon_{n}
\end{pmatrix}.
\end{aligned}
\nonumber
\end{gather}
Además, los supuestos sobre los errores se pueden expresar como \(\bm{\varepsilon}\sim N(\bm{0}, \sigma^{2}\bm{I})\), donde \(\bm{0}\) es un vector con todas las entradas iguales a cero, y \(\bm{I}\) es la matriz identidad.
...
La estimación de \(\bm{\beta}\) se hace a través del método de mínimos cuadrados ordinarios. Por lo tanto, debemos encontrar el vector \(\widehat{\bm{\beta}}\) que minimice:
\[
S(\bm{\beta}) =  \sum_{i=1}^{n}\left(y_{i} - \bm{x}_{i}'\bm{\beta}\right)^2 = \sum_{i=1}^{n}\epsilon_{i}^2.
\]
En forma matricial, tenemos:
\begin{equation}
\begin{split}
S(\bm{\beta}) &= \bm{\varepsilon}'\bm{\varepsilon}= (\bm{y}- \bm{X}\bm{\beta})'(\bm{y}- \bm{X}\bm{\beta}) \\
&= \bm{y}'\bm{y}- \bm{\beta}'\bm{X}'\bm{y}- \bm{y}'\bm{X}\bm{\beta}+ \bm{\beta}'\bm{X}'\bm{X}\bm{\beta}\\
&= \bm{y}'\bm{y}- 2\bm{\beta}'\bm{X}'\bm{y}+ \bm{\beta}'\bm{X}'\bm{X}\bm{\beta}.
\end{split}
\nonumber
\end{equation}
Por lo tanto, \(\widehat{\bm{\beta}}\) debe satisfacer:
\[
\left. \frac{\partial S}{\partial \bm{\beta}} \right|_{\widehat{\bm{\beta}}} = - 2\bm{X}'\bm{y}+ 2\bm{X}'\bm{X}\widehat{\bm{\beta}}= \bm{0}.
\]
A partir de aquí obtenemos las \textbf{ecuaciones normales}:
\[
\bm{X}'\bm{X}\widehat{\bm{\beta}}= \bm{X}'\bm{y}.
\]
En más detalle:
\begin{gather}
\begin{pmatrix}
...
\]
Note que es necesario que \(\bm{X}\) sea de rango completo, \(\mbox{rango}(\bm{X}) = p \leq n\). Esta restricción es necesaria para asegurar que \(\bm{X}'\bm{X}\) sea no singular. Si \(\bm{X}'\bm{X}\) es singular, implica que existe una combinación lineal entre las columnas de \(\bm{X}\), o que \(\mbox{rango}(\bm{X}) < p\).

El valor ajustado de \(y\) para el vector de covariables \(\bm{x}_{i}\) es \(\widehat{y}_{i}= \bm{x}_{i}'\widehat{\bm{\beta}}\). Definiendo \(\widehat{\bm{y}}= (\widehat{y}_{1},\widehat{y}_{2},\ldots,\widehat{y}_{n})\), tenemos que:
\[
\widehat{\bm{y}}= \bm{X}\widehat{\bm{\beta}}= \bm{X}(\bm{X}'\bm{X})^{-1}\bm{X}'\bm{y}= \bm{H}\bm{y}.
\]
La matriz \((n\times n)\) \(\bm{H}= \bm{X}(\bm{X}'\bm{X})^{-1}\bm{X}'\) es llamada \textbf{matriz hat} (sombrero) y desempeña un papel importante en el análisis de regresión.

Los residuos del modelo \((e_{i}=y_{i}-\widehat{y}_{i})\) también se pueden expresar en forma matricial:
\[
\bm{e}= \bm{y}- \bm{X}'\widehat{\beta}= \bm{y}- \bm{X}(\bm{X}'\bm{X})^{-1}\bm{X}'\bm{y}= \bm{y}- \bm{H}\bm{y}= (\bm{I}_{n} - \bm{H})\bm{y}.
\]
...
\begin{equation}
\begin{split}
SS_{res} &= \sum_{i=1}^{n}e^{2}_{i} = \bm{e}'\bm{e}= (\bm{y}- \bm{X}\widehat{\bm{\beta}})'(\bm{y}- \bm{X}\widehat{\bm{\beta}}) \\
&= (\bm{y}- \bm{H}\bm{y})'(\bm{y}- \bm{H}\bm{y}) = \bm{y}'(\bm{I}_{n}-\bm{H})'(\bm{I}_{n}-\bm{H})\bm{y}= \bm{y}'(\bm{I}_{n} - \bm{H})\bm{y}.
\end{split}
\nonumber
\end{equation}
...
Sabemos que \(E(\bm{y}) = \bm{X}\bm{\beta}\) y \(V(\bm{y}) = \sigma^{2}\bm{I}_{n}\), entonces:
\[
E(SS_{res}) = E\left[\bm{y}'(\bm{I}- \bm{H})\bm{y}\right] = \sigma^{2}\mbox{tr}\left( \bm{I}- \bm{H}\right) + \bm{\beta}'\bm{X}'(\bm{I}- \bm{H})\bm{X}\bm{\beta}= (n-p)\sigma^{2}.
\]
...
El valor esperado de \(\widehat{\bm{\beta}}\) es:
\begin{equation}
\begin{split}
E(\widehat{\bm{\beta}}) =& E\left[ (\bm{X}'\bm{X})^{-1}\bm{X}'\bm{y}\right] = E\left[ \bm{X}'\bm{X})^{-1}\bm{X}'(\bm{X}\bm{\beta}+ \bm{\varepsilon}) \right] \\
=& E\left[ (\bm{X}'\bm{X})^{-1}\bm{X}'\bm{X}\bm{\beta}+ (\bm{X}'\bm{X})^{-1}\bm{X}'\bm{\varepsilon}\right] = \bm{\beta}
\end{split}
\nonumber
\end{equation}
Por lo tanto, \(\widehat{\bm{\beta}}\) es un estimador insesgado de \(\bm{\beta}\) (si el modelo está bien espeficado).

La matriz de varianzas-covarianzas de \(\widehat{\bm{\beta}}\) es:
\begin{equation}
\begin{split}
V(\widehat{\bm{\beta}}) &= V\left[ (\bm{X}'\bm{X})^{-1}\bm{X}'\bm{y}\right] = (\bm{X}'\bm{X})^{-1}\bm{X}'V(\bm{y})\bm{X}(\bm{X}'\bm{X})^{-1}  \\
&= \sigma^{2}(\bm{X}'\bm{X})^{-1}\bm{X}'\bm{X}(\bm{X}'\bm{X})^{-1} = \sigma^{2}(\bm{X}'\bm{X})^{-1}.
\end{split}
\nonumber
\end{equation}
Si \(\bm{C}=(\bm{X}'\bm{X})^{-1}\), entonces \(V(\widehat{\beta}_{j}) = \sigma^{2}c_{jj}\) y \(Cov(\widehat{\beta}_{j},\widehat{\beta}_{k})=\sigma^{2}c_{jk}\), donde \(c_{jk}\) es la entrada \((j,k)\) de la matriz \(\bm{C}\).
...
Si, \(E(\bm{\varepsilon}) = \bm{0}\) y \(V(\bm{\varepsilon}) = \sigma^{2}\bm{I}_{n}\), el estimador por MCO, \(\widehat{\bm{\beta}}= (\bm{X}'\bm{X})\bm{X}'\bm{y}\), es el mejor estimador lineal insesgado de \(\bm{\beta}\). Esto quiere decir que es el estimador con menor varianza entre la clase de estimador insesgados que son combinaciones lineales de \(y\). Para la demostración, ver Sección C4 de \citet{montgomery_introduction_2012}.

Además, si \(\bm{\varepsilon}\sim N(\bm{0}, \sigma^{2}\bm{I}_{n})\), el estimador por MCO coincide con el estimador por máxima verosimilitud.
...
Para resolver estas preguntas podemos realizar pruebas de hipótesis. Generalmente, estos test requieren que \(\bm{\varepsilon}\sim N(\bm{0},\sigma^{2}\bm{I}_{n})\).
...
\tightlist
\item
  \(SS_{T} = \sum_{i=1}^{n}(y_{i}-\bar{y})^{2} = (\bm{y}- \frac{1}{n}\bm{1}'\bm{y})'(\bm{y}- \frac{1}{n}\bm{1}'\bm{y})\),
\item
  \(SS_{R} = \sum_{i=1}^{n}(\widehat{y}_{i}-\bar{y})^{2} = (\bm{H}\bm{y}- \frac{1}{n}\bm{1}'\bm{y})'(\bm{H}\bm{y}- \frac{1}{n}\bm{1}'\bm{y})\),
\item
  \(SS_{res} = \sum_{i=1}^{n}(y_{i}-\widehat{y}_{i})^{2} = (\bm{y}- \bm{H}\bm{y})'(\bm{y}- \bm{H}\bm{y})\),
\end{itemize}

y \(\bm{1}\) es un vector cuyas entradas son iguales a \(1\).
...
\[
E(MS_{res}) =  \sigma^{2} \mbox{ y }E(MS_{R}) =  \sigma^{2} + \frac{\bm{\beta}^{*'}\bm{X}_{c}'\bm{X}_{c}\bm{\beta}^{*}}{(p-1)\sigma^{2}},
\]
donde \(\bm{\beta}^{*} = (\beta_{1},\beta_{2},\ldots,\beta_{p-1})'\) y
\[
\bm{X}_{c} = \begin{pmatrix}
...
\[
\lambda = \frac{\bm{\beta}'\bm{X}_{c}'\bm{X}_{c}\bm{\beta}}{\sigma^{2}}.
\]
...
\]
donde \(c_{jj}\) es la entrada \((j,j)\) de la matriz \(\bm{C}= (\bm{X}'\bm{X})^{-1}\). Rechazamos \(H_{0}\) si \(|t_{0}| > t_{1-\alpha/2,n-p}\).
...
\bm{y}= \bm{X}\bm{\beta}+ \bm{\varepsilon},
\]
donde \(\bm{X}\) es una matrix \(n \times p\) y \(\bm{\beta}\) es el vector de coeficientes de longitud \(p\). Queremos probar si un subconjunto \(r < p\) de covariables tienen un aporte significativo en el modelo. Para esto hacemos la siguiente partición del vector \(\bm{\beta}\):
\[
\bm{\beta}= \begin{pmatrix} \beta_{0} \\ \beta_{1} \\ \vdots \\ \beta_{p-r-1} \\ \hline \beta_{p-r} \\ \beta_{p-r+1} \\ \vdots \\  \beta_{p} \end{pmatrix} =  \begin{pmatrix} \bm{\beta}_{1} \\ \hline \bm{\beta}_{2}\end{pmatrix},
\]
donde \(\bm{\beta}_{1}\) y \(\bm{\beta}_{2}\) son vector de dimensión \((p-r)\) y \((r)\), respectivamente. Por lo tanto, queremos realizar la siguiente prueba de hipótesis:
\begin{equation}
H_{0}:  \bm{\beta}_{2} = \bm{0}\qquad H_{1}:  \bm{\beta}_{2} \neq \bm{0}.
\label{eq:Hsubset}
\end{equation}
El modelo anterior se puede re-escribir de la siguiente forma:
\[
\bm{y}= \bm{X}\bm{\beta}+ \bm{\varepsilon}= \bm{X}_{1}\bm{\beta}_{1}+ \bm{X}_{2}\bm{\beta}_{2} + \bm{\varepsilon},
\]
donde \(\bm{X}_{1}\) es la matriz \(n\times (p-r)\) que contiene las columnas de \(\bm{X}\) asociadas con \(\bm{\beta}_{1}\), y \(\bm{X}_{2}\) es la matriz \(n\times r\) que contiene las columnas de \(\bm{X}\) asociadas con \(\bm{\beta}_{2}\). Este es llamado el \textbf{modelo completo}.
...
\item
  ...
  \[
  SS_{R}(\bm{\beta}) = \widehat{\beta}'\bm{X}'\bm{y}\mbox{ (con }p\mbox{ grados de libertad)}.
  \]
\item
  Cuadrado medio del error:
  \[
  MS_{res} = \frac{\bm{y}'\bm{y}- \widehat{\bm{\beta}}'\bm{X}'\bm{y}}{n-p}.
  \]
\end{itemize}

Para evaluar la contribución de los regresores asociados a \(\bm{\beta}_{2}\), ajustamos el modelo asumiendo que \(H_{0}\) es cierta. De esta forma tenemos el \textbf{modelo reducido}:
\[
...
\tightlist
\item
  Estimador de \(\bm{\beta}_{1}\):
  \[
  \widehat{\bm{\beta}}_{1} = (\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'\bm{y}.
  \]
\item
  Suma de cuadrados del modelo:
  \[
  SS_{R}(\bm{\beta}_{1}) = \widehat{\bm{\beta}}_{1}'\bm{X}_{1}'\bm{y}\mbox{ (con $p-r$ grados de libertad)}.
  \]
\end{itemize}

Entonces, la suma de cuadrados debido a \(\bm{\beta}_{2}\) dado que \(\bm{\beta}_{1}\) ya está en el modelo es:
\[
SS_{R}(\bm{\beta}_{2}| \bm{\beta}_{1}) = SS_{R}(\bm{\beta}) - SS_{R}(\bm{\beta}_{1}),
\]
con \(p-(p-r)=r\) grados de libertad. Esta suma de cuadrados es llamada la suma de cuadrados extra debido a \(\bm{\beta}\) puesto que mide el incremento en la suma de cuadrados de la regresión como resultado de adicionar los regresores \(\bm{X}_{2}\) en el modelo que ya contiene \(\bm{X}_{1}\).

Dado que \(SS_{R}(\bm{\beta}_{2}| \bm{\beta}_{1})\) y \(MS_{res}\) son independientes, podemos utilizar el siguiente estadístico de prueba:
\[
F_{0} = \frac{SS_{R}(\bm{\beta}_{2}|\bm{\beta}_{1})/r}{MS_{res}}.
\]
...
\[
\lambda = \frac{1}{\sigma^{2}}\bm{\beta}_{2}'\bm{X}_{2}'\left[ \bm{I}_{n} - \bm{X}_{1}(\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'\right]\bm{X}_{2}\bm{\beta}_{2}.
\]
Note que si hay una relación casi colineal entre \(\bm{X}_{1}\) y \(\bm{X}_{2}\) (multicolinealidad), \(\lambda\) es cercano a cero pesar que \(\bm{\beta}_{2}\) sea marcadamente distinto de cero. Es decir, que la prueba tiene poca capacidad de indicar diferencias (poco poder) en presencia de multicolinealidad. Caso contrario, el máximo poder se alcanza cuando \(\bm{X}_{1}\) y \(\bm{X}_{2}\) son ortogonales (es decir \(\bm{X}_{2}'\bm{X}_{1} = \bm{0}\)).

Entonces, si \(F_{0} > F_{1-\alpha,r,n-p}\) rechazamos \(H_{0}\) y concluimos que al menos un coeficiente en \(\bm{\beta}_{2}\) es diferente de cero. Consecuentemente, al menos una de las covariables en \(\bm{X}_{2}\) tiene un aporte significativo dentro del modelo.
...
\begin{equation}
H_{0}: \bm{T}\bm{\beta}=\bm{0}\qquad H_{1}: \bm{T}\bm{\beta}\neq \bm{0},
\label{eq:hipGeneral1}
\end{equation}
donde \(\bm{T}\) es una matriz \(m \times p\) de constantes, tal que \(r\) de las \(m\) ecuaciones de \(\bm{T}\bm{\beta}=\bm{0}\) son independientes.

El \textbf{modelo completo (FM)} es:
\[
\bm{y}=\bm{X}\bm{\beta}+\bm{\varepsilon},
\]
El estimador de \(\bm{\beta}\) es \(\widehat{\bm{\beta}}= (\bm{X}'\bm{X})^{-1}\bm{X}'\bm{y}\), y la suma de cuadrados de los residuos es \(SS_{res}(FM)\) (con \(n-p\) grados de libertad).

El \textbf{modelo reducido (RM)} se obtiene al resolver las \(r\) ecuaciones independientes de \(\bm{T}\bm{\beta}= \bm{0}\) para los \(r\) coeficientes en el modelo completo en términos de los \(p-r\) coeficientes restantes. Esto lleva al siguiente RM:
\[
\bm{y}=\bm{Z}\bm{\gamma}+\bm{\varepsilon},
\]
donde \(\bm{Z}\) es una matriz \(n\times (p-r)\) y \(\bm{\gamma}\) es un vector de dimiensión \((p-r)\) de coeficientes de regresión. La suma de cuadrados de los residuos de este modelo es \(SS_{res}(RM)\) (con \(n-p+r\) grados de libertad).
...
\]
con \(r\) grados de libertad. \(SS_{H}\) es llamado la suma de cuadrados debido a \(H_{0}:\bm{T}\bm{\beta}=\bm{c}\). El estadístico de prueba es:
\[
F_{0} = \frac{SS_{H}/r}{SS_{res}(FM)/(n-p)} = \frac{\widehat{\beta}'\bm{T}[\bm{T}(\bm{X}'\bm{X})\bm{T}']^{-1}\bm{T}\widehat{\bm{\beta}}/r}{SS_{res}(FM)/(n-p)}.
\]
Rechazamos \(H_{0}\) si \(F_{0} > F_{1-\alpha,r,n-p}\).

La hipótesis anterior se puede generalizar de la siguiente forma:
\begin{equation}
H_{0}: \bm{T}\bm{\beta}=\bm{c}\qquad H_{1}: \bm{T}\bm{\beta}\neq \bm{c},
\label{eq:hipGeneral2}
\end{equation}
Para este caso, el estadístico de prueba es:
\[
F_{0} = \frac{(\bm{T}\widehat{\bm{\beta}}-\bm{c})'[\bm{T}(\bm{X}'\bm{X})\bm{T}']^{-1}(\bm{T}\widehat{\bm{\beta}}-\bm{c})/r}{SS_{res}(FM)/(n-p)}.
\]
...
1 & 0 & 0 & 0 \\
0 & 2 & -1 & 0 \\
\end{pmatrix} \mbox{ y } \bm{c}= \begin{pmatrix} 0 \\ 3  \end{pmatrix}.
\]
Si no rechazamos \(H_{0}\), podríamos estimar \(\bm{\beta}\) sujo a las restricciones impuestar por la hipótesis nula (usando mínimos cuadrados restringidos).
...
Para que los intervalos de confianza sean válidos se requiere que se cumplan todos los supuestos del modelo, esto es \(\bm{\varepsilon}\sim N(\bm{0}, \sigma^{2}\bm{I}_{n})\).
...
\]
donde \(c_{jj}\) es la entrada \((j,j)\) de la matriz \(\bm{C}=(\bm{X}'\bm{X})^{-1}\). Entonces, el intervalo del \((1-\alpha)100\%\) de confianza para \(\widehat{\beta}_{j}\) es:
\[
...
Ahora queremos construir un intervalo de confianza para la respuesta media de \(Y\) para un punto particular \(\bm{x}_{0}=(1,x_{01},x_{02},\ldots,x_{0,p-1})\). La estimación puntual en \(\bm{x}_{0}\) es:
\[
\widehat{\mu}_{Y|\bm{x}_0} = \bm{x}_{0}'\widehat{\bm{\beta}}.
\]
Además, tenemos que \(\widehat{\mu}_{Y|\bm{x}_0} \sim N[\bm{x}_{0}\bm{\beta}, V(\widehat{\mu}_{Y|\bm{x}_0})]\) con:
\[
V(\widehat{\mu}_{Y|\bm{x}_0}) = \sigma^{2}\bm{x}_{0}'(\bm{X}'\bm{X})^{-1}\bm{x}_{0}.
\]
Por lo tanto, el intervalo del \((1-\alpha)100\%\) de confianza para \(E(Y|\bm{x}_{0})\) es:
\[
\widehat{\mu}_{Y|\bm{x}_0} \pm t_{1-\alpha/2,n-p}\sqrt{MS_{res}\bm{x}_{0}'(\bm{X}'\bm{X})^{-1}\bm{x}_{0}}.
\]
De igual forma, el intervalo del \((1-\alpha)100\%\) de confianza para una observación futura en \(\bm{x}_{0}\) es:
\[
\widehat{\mu}_{Y|\bm{x}_0} \pm t_{1-\alpha/2,n-p}\sqrt{MS_{res}\left[1+ \bm{x}_{0}'(\bm{X}'\bm{X})^{-1}\bm{x}_{0}\right]}.
\]
...
{\centering \includegraphics{bookdownproj_files/figure-latex/extrapolacionOculta-1} 
...
Una aproximación de la RVH es a través de la matriz \(\bm{H}\). El conjunto de puntos \(\bm{x}\) que satisfacen, \(\bm{x}'(\bm{X}'\bm{X})^{-1}\bm{x}\leq \max(h_{ii})\), prudcen un elipsoide que encierra todos los puntos dentro de la \(RVH\). Entonces, un punto de predicción \(\bm{x}_{0}\) está fuera de la RVH si \(h_{00} > \max{h_{ii}}\), donde:
\[
h_{00} = \bm{x}'_{0}(\bm{X}'\bm{X})^{-1}\bm{x}_{0}.
\]
...
La Figura \ref{fig:birthweightExtrapolacion} muestra el gráfico de dispersión de las covariables, donde los puntos rojos indican los valores donde se quieren hacer predicciones. Aquí vemos que en los puntos \(\bm{x}_{02}\) y \(\bm{x}_{03}\) no estaríamos extrapolando. Pero, es difícil de determinar para los puntos \(\bm{x}_{01}\) y \(\bm{x}_{04}\). Para esto vamos a calcular las aproximaciones de la RVH y verificar si en estos puntos estaríamos extrapolando.

\begin{figure}

{\centering \includegraphics{bookdownproj_files/figure-latex/birthweightExtrapolacion-1} 
...
Para la predicción en el punto \(\bm{x}_{01}\), tenemos que \(h_{0} = (1, 34, 77)(\bm{X}'\bm{X})^{-1}(1, 34, 77)' = 0.3487\) \(>\) \(h_{max} = 0.2276\). Por lo tanto, aquí se estaría extrapolando. Para el resto de punto no hay problemas de extrapolación.
...
\[
y_{i}^{*} = b_{1}z_{i1} + b_{2}z_{i2} + \ldots + b_{p-1}z_{i,p-1} = \bm{z}_{i}'\bm{b}+ \bm{\varepsilon}.
\]
El estimador por MCO es:
\[
\widehat{\bm{b}}= (\bm{Z}'\bm{Z})^{-1}\bm{Z}'\bm{y}^{*}.
\]
Note que con este escalamiento, la matriz \((\bm{Z}'\bm{Z})\) es igual a la matriz de correlación de las covariables. Esto es:
\[
(\bm{Z}'\bm{Z}) = \bm{R}= \begin{pmatrix}
...
\end{pmatrix},
\]
donde \(r_{jk}\) es la correlación entre las covariables \(x_{j}\) y \(x_{k}\). Además, la matriz \(\bm{Z}'\bm{y}^{*}\) es el vector de correlación entre la variable respuesta y cada covariable. Esto es:
\[
\bm{Z}'\bm{y}^{*} = (r_{1y},r_{2y},r_{3y},\ldots,r_{p-1,y})',
\]
...
Recordemos que el estimador por MCO es \(\widehat{\bm{\beta}}=(\bm{X}'\bm{X})^{-1}\bm{X}'\bm{y}\). Por lo tanto es necesario que la matriz \(\bm{X}'\bm{X}\) sea no singular. En caso contrario, no es posible encontrar la inversa y las ecuaciones normales no tendrán una única solución. Cuando sucede esto se debe a que hay al menos una columna de \(\bm{X}\) linealmente dependiente.

En regresión se utiliza las palabras multicolinealidad cuando hay una dependencia aproximada en las columnas de \(\bm{X}\). Es decir que al menos una covariable puede representarse, de forma aproximada, como una relación lineal de las otras:
\[
...
\[
\bm{X}_{1} = \begin{pmatrix}
1& 1 \\ 
1 & 5 \\ 
2 & 1 \\ 
2 & 5 \\
\end{pmatrix} \mbox{ y }
\bm{X}_{2} = \begin{pmatrix}
1& 1 \\ 
1 & 2 \\ 
2 & 4 \\ 
2 & 5 \\
...
\[
\bm{Z}_{1}'\bm{Z}_{1} = \begin{pmatrix}
1 & 0 \\ 0 & 1
\end{pmatrix} \mbox{ y }
\bm{Z}_{1}'\bm{Z}_{1} = \begin{pmatrix}
1 & 0.95 \\ 0.95 & 1
\end{pmatrix}.
\]
Por lo tanto, la varianza de \(\widehat{\bm{b}}\) para ambos casos es:
\[
V(\widehat{\bm{b}}_{1}) = \sigma^{2}(\bm{Z}_{1}'\bm{Z}_{1})^{-1} =  \sigma^{2} \begin{pmatrix}
1 & 0 \\ 0 & 1
...
y
\[
V(\widehat{\bm{b}}_{2}) = \sigma^{2}_{0}(\bm{Z}_{2}'\bm{Z}_{2})^{-1} =  \sigma^{2} \begin{pmatrix}
1 & 0.95 \\ 0.95 & 1
\end{pmatrix}^{-1} = \sigma^{2} \begin{pmatrix}
10 & 9.49 \\ 9.49 & 10
\end{pmatrix}.
\]
Aquí podemos ver que la varianza de \(\widehat{\bm{b}}_{2}\) está inflada debido a la alta correlación entre las columnas de \(\bm{X}_{2}\). Es 10 veces mayor que la varianza de \(\widehat{\bm{b}}_{1}\) (las columnas de \(\bm{X}_{1}\) son independientes).

En el ejemplo anterior vemos que los valores de la diagonal de la matriz \((\bm{Z}'\bm{Z})^{-1}\) nos indican en cuanto aumenta la varianza de las estimaciones de los coeficientes debido a la multicolinealidad. Por esta razón, estos valores toman el nombre de \textbf{factores de inflación de varianza (VIFs)} y son uno de los indicadores para el diagnostico de este problema.
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Icecream-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/bassFig-1} 
...
Si \(E(\bm{\varepsilon}) = \bm{0}\), entonces \(E(\bm{y}| \bm{X}) = \bm{X}\bm{\beta}\), y el estimador por MCO es insesgado. Sin embargo, si omitimos variables relevantes dentro del modelo las estimaciones serán sesgadas. Para ver esto, supongamos que el modelo generador de los datos es:
\[
\bm{y}= \bm{X}\bm{\beta}+ \bm{\varepsilon}=  \bm{X}_{1}\bm{\beta}_{1} + \bm{X}_{2}\bm{\beta}_{2} + \bm{\varepsilon},
\]
donde las columnas de la matriz \(n\times p\) de covariables \(\bm{X}\) está divida en dos submatrices \(\bm{X}_{1}\) y \(\bm{X}_{2}\) de dimensiones \(n \times (p-r)\) y \(n\times r\), respectivamente. Además, asumimos que \(\bm{\varepsilon}\sim N(\bm{0}, \sigma^{2}\bm{I}_{n})\).

Ahora, consideramos estimar el siguiente modelo:
\[
\bm{y}= \bm{X}_{1}\bm{\beta}_{1} + \bm{\varepsilon}^{*},
\]
es decir estamos omitiendo las covariables contenidas en \(\bm{X}_{2}\). El estimador de \(\bm{\beta}_{1}\) es:
\[
\widehat{\bm{\beta}}_{1} = (\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'\bm{y},
\]
y el estimador de \(\sigma^{2}\) es:
\[
\widehat{\sigma}^{2}_{1} = \frac{\bm{y}'(\bm{I}_{n} - \bm{H}_{1})\bm{y}}{n-(p-r)}, \mbox{ donde }\bm{H}_{1} = \bm{X}_{1}(\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'.
\]
El valor esperado de \(\widehat{\bm{\beta}}_{1}\) es:
\begin{equation}
\begin{split}
E(\widehat{\bm{\beta}}_{1}) &= E\left[(\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'\bm{y}\right] = (\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'E(\bm{y}) \\
&= (\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'E(\bm{X}_{1}\bm{\beta}_{1} + \bm{X}_{2}\bm{\beta}_{2} + \bm{\varepsilon}) = \bm{\beta}_{1} +  (\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'\bm{X}_{2}\bm{\beta}_{2}.
\end{split}
\nonumber
\end{equation}
Evidentemente, el sesgo de \(\widehat{\bm{\beta}}_{1}\) depende de la magnitud de \(\bm{\beta}_{2}\). Entre más importante sean los efectos asociados a las covariables omitidas \((\bm{\beta}_{2})\), mayor será el sesgo. Si las columnas de \(\bm{X}_{1}\) son ortogonales de las columnas de \(\bm{X}_{2}\), tenemos que \(\bm{X}_{1}'\bm{X}_{2} = \bm{0}\). Así que, en este caso particular, \(\widehat{\bm{\beta}}_{1}\) es insesgado (así omitamos las covariables en \(\bm{X}_{2}\)).

El valor esperado de \(\widehat{\sigma}^{2}_{1}\) es:
\[
E(\widehat{\sigma}^{2}_{1}) = \sigma^{2} + \frac{\bm{\beta}_{2}'\bm{X}'_{2}(\bm{I}_{n} - \bm{H}_{1})\bm{X}_{2}\bm{\beta}_{2}}{n-p_{1}}.
\]
Dado que \((\bm{I}- \bm{H}_{1})\) es idempotente y, por lo tanto, positiva semi-definida, entonces \(E(\widehat{\sigma}_{1}^{2}) > \sigma^{2}\). Esto quiere decir que \(\widehat{\sigma}^{2}_{1}\) es un estimador sesgado de \(\sigma^{2}\)

Ahora veamos el efecto de omitir covariables relevantes sobre las predicciones de \(y\) en el punto \(\bm{x}_{0} = (\bm{x}_{01}', \bm{x}_{02}')'\). Tenemos que:
\[
\widehat{\bm{y}}_{0} = \bm{x}_{01}'\widehat{\bm{\beta}}_{1} = \bm{x}_{01}'(\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'\bm{y}.
\]
El valor esperado de \(\widehat{\bm{y}}_{0}\) es:
\[
E(\widehat{\bm{y}}_{0}) = \bm{x}_{01}'E(\widehat{\bm{\beta}}_{1}) = \bm{x}_{01}'\left[ \bm{\beta}_{1} +  (\bm{X}_{1}'\bm{X}_{1})^{-1}\bm{X}_{1}'\bm{X}_{2}\bm{\beta}_{2} \right]. 
\]
Por lo tanto las predicciones también son sesgadas, \(E(\widehat{\bm{y}}_{0}) \neq \bm{x}_{01}\bm{\beta}_{1} + \bm{x}_{02}\bm{\beta}_{2}\).
...
Cuando \(V(\bm{\varepsilon}) = \sigma^{2}\bm{V}\), pero asumimos erroneamente que \(V(\bm{\varepsilon}) = \sigma^{2}\bm{I}_{n}\), el estimador \(\widehat{\bm{\beta}}\) sigue siendo insesgado. Pero, tenemos que:
\begin{equation}
\begin{split}
V(\widehat{\bm{\beta}}) &= V\left[ (\bm{X}'\bm{X})^{-1}\bm{X}'\bm{y}\right] = (\bm{X}'\bm{X})^{-1}\bm{X}'V(\bm{y})\bm{X}(\bm{X}'\bm{X})^{-1} \\
&= \sigma^{2}(\bm{X}'\bm{X})^{-1}\bm{X}'\bm{V}\bm{X}(\bm{X}'\bm{X})^{-1},
\end{split}
\nonumber
\end{equation}
es, generalmente, diferente de \(\sigma^{2}(\bm{X}'\bm{X})^{-1}\) (la varianza que asumimos como cierta). Igualmente, el estimador por MCO pierde su condición de optimalidad. Es decir, deja de ser el mejor estimador lineal insesgado.

El estimador de \(\sigma^{2}\) es sesgado:
\[
E(\widehat{\sigma}^{2}) = \frac{\sigma^{2}}{n-p}E\left[\bm{y}'(\bm{I}_{n} - \bm{H})\bm{y}\right] = \frac{\sigma^{2}}{n-p} \mbox{tr}\left[ \bm{V}(\bm{I}_{n}-\bm{H})\right].
\]
Las predicciones son insesgadas, pero:
\[
V(\widehat{y}_{0}) = V(\bm{x}_{0}'\widehat{\bm{\beta}}) = \sigma^{2}\bm{x}_{0}'(\bm{X}'\bm{X})^{-1}\bm{X}'\bm{V}\bm{X}(\bm{X}'\bm{X})^{-1} \bm{x}_{0},
\]
que es diferente de \(\sigma^{2}\bm{x}_{0}'(\bm{X}'\bm{X})^{-1}\bm{x}_{0}\) (la varianza que asumimos como cierta).

Cuando \(\bm{V}\) es una matriz diagonal:
\[
\bm{V}= \begin{pmatrix}
v_{11} & 0 & 0 & \ldots & 0 \\
0 & v_{22} & 0 & \ldots & 0 \\
...
\end{pmatrix},
\]
tenemos que hay \textbf{heterocedasticidad}. Cada error tiene una varianza diferente \(V(\varepsilon_{j})=\sigma^{2}v_{jj}\), pero están incorrelacionados. Si los valores fuera de la diagonal de \(\bm{V}\) son diferentes de cero, entonces los errores están correlacionados.
...
\[
e_{i} = y_{i}- \widehat{y}_{i}, \mbox{ en forma matricial }\bm{e}= \bm{y}- \widehat{\bm{y}}= (\bm{I}_{n} - \bm{H})\bm{y}.
\]
...
\[
\bm{e}= (\bm{I}_{n} - \bm{H})(\bm{X}\bm{\beta}+\bm{\varepsilon}) = (\bm{I}_{n} - \bm{H})\bm{\varepsilon}.
\]
Por lo tanto toda desviación de las premisas de los errores se debe reflejar en los residuales. Si \(\bm{\varepsilon}\sim N(\bm{0},\sigma^{2}\bm{I}_{n})\), entonces:
\[
\bm{e}\sim N \left[ \bm{0}, \sigma^{2}(\bm{I}_{n} - \bm{H})  \right].
\]
...
\]
Entonces, \(r_{i}\) tiene varianza constante (\(V(r_{i})=1\)) independiente del lugar de \(\bm{x}_{i}\).
...
{\centering \includegraphics{bookdownproj_files/figure-latex/PatronesResiduos-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/patronesResiduos2-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/residuosBWdata-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/residuosBWdata2-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/qqplots-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/qqnormBWdata1-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/residuosCorr-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/heladosResiduos1-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/heladosResiduos2-1} 
...
El objetivo de la prueba de falta de ajuste es determinar si la relación entre la variable respuesta y las covariables puede asumirse cómo lineal. Hay que tener en cuenta que esta prueba es sensible a ajamientos de los supuestos de normalidad, varianza constante e independencia de los errores. Además, requiere que se tengan mútiple observaciones de \(y\) para diferentes niveles de \(\bm{x}\). Por ejemplo, el test puede implementarse en los datos de la longitud de los peces puesto que tenemos varios individuos con las mismas edades. En los otros casos, no es posible, por lo menos de forma exacta. El objetivo de estas observaciones replicadas es tener una estimación independiente de \(\sigma^{2}\).
...
{\centering \includegraphics{bookdownproj_files/figure-latex/pecesResiduos1-1} 
...
Algunas pruebas de heterocedasticidad asumen que la varianza de los errores se compone de una parte constante y otra que varía según unas variables \((\bm{z})\):
\[
\sigma^{2}_{i} = f(\sigma^{2},\bm{z}_{i}),
\]
donde \(\sigma^{2}\) es la parte fija de la varianza, \(\bm{z}_{i}\) el conjunto de variables cuyos valores se asocian con los cambios en la varianza de los errores. Por lo general se asume que la función de varianza depende de algunas de las covariables del modelo, es decir que \(\bm{z}_{i}=\bm{x}_{i}\).
...
Las últimas tres pruebas requieren que se tengan múltiples observaciones para cada nivel de \(\bm{x}\). En caso que no se tengan repeticiones, es posible agruparlas por los vecinos más cercanos e implementar la prueba de forma aproximada.
...
\[
y_{t} = \bm{x}_{t}'\bm{\beta}+ \varepsilon_{t}, \mbox{ con }\varepsilon_{t}= \phi\varepsilon_{t-1} + a_{t},
\]
donde \(y_{t}\) y \(\bm{x}_{t}\) son la variable respuesta observada y el conjunto de covariables observadas en el tiempo \(t\), respectivamente, y \(\phi\) es el parámetro de autocorrelación \((|\phi| < 1)\). Además, se asume que \(a_{t} \sim N(0,\sigma^{2}_{a})\) y \(cov(a_{j},a_{k})=0\), para todo \(j \neq k\). A partir de estos resultados, se tiene que:
\[
...
\item
  A la hora de validar el supuesto de normalidad de los errores se está interesado en saber si el alejamiento de ese modelo normal, es aceptable desde el punto de vista de la conservación de las propiedades y ventajas que se heredan de la normalidad. Las estimaciones de \(\widehat{\bm{\beta}}\) son generalmente robustas a desviaciones de la normalidad (Teorema del límite central).
\end{itemize}
...
{\centering \includegraphics{bookdownproj_files/figure-latex/UNdataFig-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Un11res-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Un11resPartial-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Educdata-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Educres-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/linealPat-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/UNtransFig-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/UNtrans2Fig-1} 
...
\[
\ell(\bm{\beta},\sigma^{2} | \lambda) = -\frac{2}{n}\log \left( 2\pi\sigma^{2}\right) - \frac{1}{2\sigma^{2}}\sum_{i=1}^{n}\left( y_{i}^{\lambda} - \bm{x}_{i}'\bm{\beta}\right)^{2} = -\frac{2}{n}\log \left( 2\pi\sigma^{2}\right) - \frac{1}{2\sigma^{2}}SS_{res}(\lambda).
\]
...
\[
G_{0}^{2} = -2\left[ \ell(\bm{\beta},\sigma^{2} | \lambda=1) - \ell(\bm{\beta},\sigma^{2} | \lambda=\widehat{\lambda}) \right].
\]
...
\[
 \ell(\bm{\beta},\sigma^{2} | \lambda) \geq  \ell(\bm{\beta},\sigma^{2} | \lambda=\widehat{\lambda}) - \frac{1}{2}\chi^{2}_{1,1-\alpha}.
\]
...
{\centering \includegraphics{bookdownproj_files/figure-latex/educLambda-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/EducBC2Fig-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/EducBC3Fig-1} 
...
\begin{equation}
\bm{y}= \bm{X}\bm{\beta}+ \bm{\varepsilon}, \mbox{ donde }\bm{\varepsilon}\sim N(\bm{0},\sigma^{2}\bm{V}),
\label{eq:modV}
\end{equation}
donde \(\bm{V}\) es una matriz diagonal \((n \times n)\):
\[
\bm{V}= \begin{pmatrix}
v_{11} & 0 & 0 & \ldots & 0 \\
0 & v_{22} & 0 & \ldots & 0 \\
...
\end{pmatrix}.
\]
Esto quiere decir que tenemos presencia de hetorocedasticidad, donde \(V(\varepsilon_{i}) = \sigma^{2}v_{ii}\). Recordemos que si aplicamos el estimador por MCO, las estimaciones siguen siendo insesgadas pero son ineficientes. Además, las varianzas estimadas de \(\widehat{\bm{\beta}}\), y de las predicciones, están mal calculadas. Lo que puede afectar la cobertura de los intervalos de confianza y el nivel de signficancia de las pruebas de hipótesis.

La función de verosimilitud del modelo \eqref{eq:modV}, asumiendo que \(\bm{V}\) es conocida, es:
\[
L(\bm{\beta},\sigma^{2} | \bm{V}) =  \frac{1}{(2\pi)^{n/2}|\sigma^{2}\bm{V}|^{1/2}}\exp \left[-\frac{1}{2\sigma^{2}}(\bm{y}- \bm{X}'\bm{\beta})'\bm{V}^{-1}(\bm{y}- \bm{X}'\bm{\beta}) \right].
\]
Por lo que para encontrar el estimador de \(\bm{\beta}\) debemos minimizar la suma de cuadrados ponderada:
\begin{equation}
SS_{Wres} = (\bm{y}- \bm{X}'\bm{\beta})'\bm{W}(\bm{y}- \bm{X}'\bm{\beta}) = \sum_{i=1}^{n} w_{ii}(y_{i} - \bm{x}_{i}'\bm{\beta})^{2},
\label{eq:SSWE}
\end{equation}
donde \(\bm{W}= \bm{V}^{-1}\), es decir que \(w_{ii}= \frac{1}{v_{ii}}\). Note que las observaciones con mayor varianza tienen menor peso en la estimación de \(\bm{\beta}\).

Al minimizar \eqref{eq:SSWE} se obtiene el estimador por MCP:
\[
\widehat{\bm{\beta}}_{W} = (\bm{X}'\bm{W}\bm{X})^{-1}\bm{X}'\bm{W}\bm{y}.
\]
Además,
\begin{equation}
V(\widehat{\bm{\beta}}_{W}) = \sigma^{2}(\bm{X}'\bm{W}\bm{X})^{-1}.
\label{eq:VarBetaW}
\end{equation}
Si \(\bm{V}\) está correctamente especificada, se puede probar que \(\widehat{\bm{\beta}}_{W}\) es el mejor estimador insesgado de \(\bm{\beta}\).

Los residuos del ajuste del modelo son:
\[
e_{Wi} = \sqrt{w_{ii}}(y_{i} - \widehat{y}_{i}), \mbox{ de forma matricial }\bm{e}_{W} = \bm{W}^{1/2}(\bm{y}- \bm{X}\widehat{\bm{\beta}}_{W}),
\]
donde \(\bm{W}= \bm{W}^{1/2}\bm{W}^{1/2}\). De este resultado obtenemos el estimador insesgado de \(\sigma^{2}\):
\[
\widehat{\sigma}^{2} = \frac{1}{n-p}\sum_{i=1}^{n}w_{ii}(y_{i}-\bm{x}_{i}'\widehat{\bm{\beta}})^{2} = \frac{1}{n-p}(\bm{y}- \bm{X}\widehat{\bm{\beta}})'\bm{W}(\bm{y}- \bm{X}\widehat{\bm{\beta}}).
\]
...
Es común que \(\sigma^{2}_{i}\) varíe de acuerdo a una o varias covariables, o con respecto \(E(y_{i}|\bm{x}_{i})\). Por ejemplo, en los datos de educación parece que la varianza incrementa con \(E(y_{i}| \bm{x}_{i})\). En este caso podemos aplicar el siguiente procedimiento:
...
Si \(\widehat{\bm{\beta}}_{W}\) difiere mucho de \(\widehat{\bm{\beta}}\), es posible repetir el proceso anterior una vez más (mínimos cuadrados iterativamente ponderados).
...
{\centering \includegraphics{bookdownproj_files/figure-latex/EducMCPFig-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Un11resFig-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/puntosAI-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/puntosAIw-1} 
...
Entonces, para detectar valores influyentes vamos a centrarnos en la matriz \textit{hat} \((\bm{H})\). El elemento \(h_{ij}\) de esta matriz se calcula como:
\begin{equation}
h_{ij} = \bm{x}_{i}'(\bm{X}'\bm{X})\bm{x}_{j}.
\label{eq:hij}
\end{equation}
Algunas propiedades de la matriz \(\bm{H}\) son:

\begin{itemize}
\item $\sum_{i=1}^{n} = p$.
\item $\sum_{i=1}^{n}h_{ij} = \sum_{j=1}^{n}h_{ij} = 1$
\item Cada valor $h_{ii}$ está acotado entre $1/n$ y $1/r$ ($r$ es el número de columnas de $\bm{X}$ iguales a $\bm{x}_{i}$).
\end{itemize}

Además, la diagonal de la matriz \(\bm{H}\) es una medida estandarizada de la distancia de las observaciones al centro (centroide) del espacio de \(\bm{x}\). Por lo tanto, valores altos en la diagonal de \(\bm{H}\) pueden indicarnos observaciones que son potencialmente influyentes porque están alejadas en el espacio de las covariables. Viendo \eqref{eq:hij}, si \(h_{ii}\) es muy cercano a uno, \(\widehat{y}_{i}\) estará muy cerca de \(y_{i}\) (dado que el peso de las demás observaciones será casi cero).
...
La Figura \ref{fig:Un11Hdiag}(a) muestra los valores de la diagonal de la matrix \(\bm{H}\). Aquí podemos observar que algunos países presentan valores más altos del punto de corte (\(2\frac{p}{n} = 0.0302\)). Comparado con los demás valores, el valor asociado a Trinidad y Tobago es muy alto. Por lo cuál este país lo podemos considerar como un punto de balanceo.
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Un11Hdiag-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Un11valAtip-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Un11valAtipElim-1} 
...
De igual forma se podría evaluar cuanto cambian las estimaciones de \(E(y|\bm{x}_{i})\) o las varianzas de los coeficientes \(V(\beta_{j})\) al eliminar observaciones una a una.
...
Esta es una medida de la distancia entre las estimaciones por MCO basado en los \(n\) puntos (\(\hat{\bm{\beta}}\)), y el estimado obtenido eliminando el \(i\)-ésimo punto (\(\hat{\bm{\beta}}_{(i)}\)). Es decir que es un indicador global de cuanto cambian todas las estimaciones de los coeficientes de regresión en conjunto. Esta medida se expresa de la siguiente forma:

\begin{equation}
\begin{split}
D_{i} &= \frac{(\hat{\bm{\beta}}_{(i)}-\hat{\bm{\beta}})'\bm{X}'\bm{X}(\hat{\bm{\beta}}_{(i)}-\hat{\bm{\beta}})'}{p \hat{\sigma}^{2}}   \\
      &= \frac{(\hat{\bm{y}}_{(i)} - \hat{\bm{y}} )'(\hat{\bm{y}}_{(i)} - \hat{\bm{y}} )}{p\hat{\sigma}^{2}} = \frac{r_{i}^{2}}{p}\frac{h_{ii}}{1-h_{ii}}, \qquad i=1,2,\ldots,n.
\end{split} \nonumber
\end{equation}

Aquí podemos observar que \(D_{i}\) consta de dos componentes, uno asociado a el residuo \((r_{i})\) y otro a la distancia del vector \(\bm{x}_{i}\) al centroide de la matriz de las covariables. Ambos (o alguno de ellos) puede contribuir a valores altos de este indicador.

Entonces, los puntos asociados a valores altos de \(D_{i}\) tienen gran influencia sobre la estimación de \(\bm{\beta}\) por MCO. Se considera como un punto influyente si tiene asociado un \(D_{i} > 4/n\) (algunos textos sugieren \(D_{i} > 1\)).
...
\[
DFBETAS_{(i,j)} = \frac{\hat{\beta}_{j} - \hat{\beta}_{j(i)}}{\hat{\sigma}^{2}_{(i)}C_{jj}} = \frac{r_{(j,i)}}{\sqrt{\bm{r}_{j}'\bm{r}_{j}}}\frac{t_{i}}{\sqrt{1-h_{ii}}},
\]

donde \(C_{jj}\) es el \(j\)-ésimo valor de la diagonal de \((\bm{X}'\bm{X})^{2}\). \(\bm{r}_{j}\) es la \(j\)-ésima fila de \(\bm{R}=(\bm{X}'\bm{X})^{-1}\bm{X}\).
...
Los diagnósticos anteriores permiten ver el efecto de las observaciones sobre \(\hat{\bm{\beta}}\) o \(\hat{\bm{y}}\). Pero no proporcionan información sobre la precisión general de la estimación. Una medida global de la precisión es la varianza generalizada:
...
\[
COVRATIO_{i} = \frac{|(\bm{X}'_{(i)}\bm{X}_{(i)})^{-1}\hat{\sigma}_{(i)}^{2}|}{| (\bm{X}'\bm{X})^{-1}\hat{\sigma}^{2} |}= \left( \frac{S^{2}_{(i)}}{S^{2}} \right)^{p} \left( \frac{1}{1-h_{ii}} \right).
\]

Si \(COVRATIO_{i} > 1 + 3p/n\) o \(COVRATIO_{i} > 1 - 3p/n\) se debería considerar a la \(i\)-ésima observación como influyente para la precisión de \(\widehat{\bm{\beta}}\).
...
{\centering \includegraphics{bookdownproj_files/figure-latex/Un11Cook-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/UNdataDfBetas-1} 
...
{\centering \includegraphics{bookdownproj_files/figure-latex/UNdataOtros-1} 
