...
La Figura \@ref(fig:BWdata2) muestra la relación entre las variables de estudio. Aquí podemos observar una relación lineal positiva fuerte entre el peso al nacer y la edad gestacional (correlación igual a `r round(cor(birthweight[,c(3,4)])[1,2],2)`). La relación entre el peso al nacer y el peso de la madre es lineal positiva, aunque no tan fuerte como la anterior (correlación igual a `r round(cor(birthweight[,c(3,8)])[1,2],2)`). 

La Figura \@ref(fig:BWdata2) y la matriz de correlación se pueden hacer con los siguientes códigos:
```{r BWdata2, fig.height = 4.5, fig.width = 6,fig.align = "center",fig.cap = "Gráfico de dispersion del peso del recien nacido y la edad gestacional."}
birthweight = read.csv("birthweight.csv",header = T)
...
```{r extrapolacionOculta, echo=F, fig.height = 4.5, fig.width = 6,fig.align = "center",fig.cap = "Ejemplo de extrapolación en regresión múltiple",warning=FALSE,message = FALSE}
library(car)
